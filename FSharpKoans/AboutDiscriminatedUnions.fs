...
module ``07: The Good Kind of Discrimination`` = 
    ...
    let ``04 Pattern-matching using named fields`` () =
        let result =
            ...
            | BSc(x,y) -> "correct" // <-- USE a pattern-match with named fields!
            | _ -> "nope"
    ...
    let ``06 A discriminated union can refer to itself (i.e., it can be recursive).`` () =
        ...
        let a = 
            ...
            let node5 = Node ("nje",node3,node4) // <-- you may want to spread this over multiple lines and/or let-bindings ...!
            node5
