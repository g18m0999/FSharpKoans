    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int =
          let rec reclen xs count = 
            match xs with 
            |[] -> count 
            |_::rest -> reclen rest (count + 1)
          reclen xs 0 // function to find the length of a list
    ...
    let ``03 Reversing a list, the hard way`` () =
        let rev (xs : 'a list) : 'a list =
            let rec revList xs rev =
                match xs with 
                |[] -> rev
                |a::rest -> revList rest (a::rev)
            revList xs []// write a function to reverse a list here.
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            let rec addList xs alist =
                match xs with
                |[] -> alist
                |a::rest -> addList rest (alist@[a+1])
            addList xs []// write a function which adds 1 to each element
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
            let rec MultList xs mlist =
                match xs with
                |[] -> mlist
                |a::rest -> MultList rest (mlist@[a*2])
            MultList xs [] // write a function which doubles each element
   ...
   *)
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
          let rec maplist xs plist =
            match xs with 
            | [] -> plist 
            |a::rest -> maplist rest (plist@[f a])
          maplist xs []// write a map which applies f to each element
    ...
    let ``11 Fixed-function filtering, the hard way`` () =
        let filter (xs : int list) : int list =
            ...
            match xs with 
            |[] -> []
            |_ -> filterlist (fun x -> x%2 = 1) xs  // write a function to filter for odd elements only.
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
            ...
            |_ -> foldfunc xs initialState // write a function to do what's described above
        ...
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            let rec foldlist list acc = 
                match list with 
                   ...
                   |_ -> acc         
            match initialState with 
            |_ -> foldlist xs initialState  // write a function to do a fold.
        fold (+) 0 [1;2;3;4] |> should equal 10
    ...
    let ``18 exists: finding whether any matching item exists`` () =
        let exists (f : 'a -> bool) (xs : 'a list) : bool =
            let rec existlist list  = 
                match list with
                |[] -> false
                |a::b -> match f a with
                         |false -> existlist b
                         |true -> true
            existlist xs // Does this: https://msdn.microsoft.com/en-us/library/ee370309.aspx
    ...
    let ``19 partition: splitting a list based on a criterion`` () =
        let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) =
            let rec partitionlist list (passed,failed) = 
             match list with 
            ...
            partitionlist xs ([],[]) // Does this: https://msdn.microsoft.com/en-us/library/ee353782.aspx
    ...
    let ``20 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
            let rec initlist len acc = 
                match len < n with
                |true -> initlist (len+1) (f len::acc)
                |false -> List.rev acc
            initlist 0 [] // Does this: https://msdn.microsoft.com/en-us/library/ee370497.aspx
    ...
    let ``21 tryFind: find the first matching element, if any`` () =
        let tryFind (p : 'a -> bool) (xs : 'a list) : 'a option =
          let rec tryFindfunc list acc = 
            match list with
            |[] -> None
            |a::b -> match (p a) with
                        |true -> Some a
                        |false -> tryFindfunc b acc
          tryFindfunc xs None// Does this: https://msdn.microsoft.com/en-us/library/ee353506.aspx
    ...
    let ``22 tryPick: find the first matching element, if any, and transform it`` () =
        let tryPick (p : 'a -> 'b option) (xs : 'a list) : 'b option =
            let rec tryPickfunc list =
                match list with 
                |[] -> None
                |a::b -> match (p a) with
                         |None -> tryPickfunc b
                         |Some _ -> p a
            tryPickfunc xs // Does this: https://msdn.microsoft.com/en-us/library/ee353814.aspx
        let f x =
            match x<=45 with
            | true -> Some(x*2)
            | _ -> None
    ...
    *)
    ...
    let ``23 choose: find all matching elements, and transform them`` () =
        ...
        let choose (p : 'a -> 'b option) (xs : 'a list) : 'b list =
            let rec chooseFunc list acc =
                match list with
                |[] -> List.rev acc
                |a::b -> match p a with
                         |None -> chooseFunc b acc
                         |Some a -> chooseFunc b  (a::acc)
            chooseFunc xs [] // Does this: https://msdn.microsoft.com/en-us/library/ee353456.aspx
        let f x =
            match x<=45 with
            | true -> Some(x*2)
            | _ -> None
    ...
    let ``24 mapi: like map, but passes along an item index as well`` () =
        let mapi (f : int -> 'a -> 'b) (xs : 'a list) : 'b list =
            let rec mapiFunc list acc len =
                match list with 
                |[] ->List.rev acc
                |a::b -> mapiFunc b (f len a::acc) (len+1)
            mapiFunc xs [] 0 // Does this: https://msdn.microsoft.com/en-us/library/ee353425.aspx
