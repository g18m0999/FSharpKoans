...
*)
   ...
   *)
    ...
    let ``08 An identifier pattern will match anything`` () =
        let x = 72 // replace with an integer
        let y = "Frank" // replace with a string
        let z = 0.1124 // replace with anything else!
        x |> should be ofType<int>
        y |> should be ofType<string>

    [<Test>]
    let ``09 A wildcard pattern will match anything`` () =
        let _ = 78 // replace with an integer
        let _ = "Sara" // replace with a string
